...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export pipeline tables to Parquet")
    parser.add_argument("--db-path", type=str, default=DEFAULT_DB_PATH)
    parser.add_argument("--out-dir", type=str, default="data/parquet")
    ...
    try:
        ...
        for cfg in TABLE_CONFIG:
            if cfg.name not in tables:
                ...
                continue
            entry = (
                export_partitioned(conn, cfg, args.out_dir)
                if cfg.partition_col
                else export_single(conn, cfg, args.out_dir)
            )
        ...
        for table in tracking_tables:
            ...
            entry = export_single(conn, cfg, args.out_dir)
            manifest_tables.append(entry)
    finally:
        conn.close()

    write_manifest(args.out_dir, manifest_tables)
